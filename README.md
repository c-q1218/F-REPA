...
accelerate launch train_FA.py \
  ...
  --proj-coeff=1 \
  --encoder-depth=6 \
  --output-dir="/mnt/ssd/model/image/repa/FA" \
  --exp-name="b-9-b-enc6-lambda1" \
  --data-dir="/mnt/ssd/dataset/image/imagenet-1k/data"
```
################lambda 1, 0.5      layer 5
...
torchrun --nnodes=1 --nproc_per_node=4 generate.py \
  ...
  --ckpt "/mnt/ssd/model/image/repa/FA/9-b-enc8-lambda1/checkpoints/0400000.pt" \
  --path-type=linear \
  --encoder-depth=8 \
  ...
  --cfg-scale=0 \
  --guidance-high=0.7 \
  --sample-folder-dir="/mnt/ssd/dataset/image/imagenet-1k/generated-result/modified-FA-9-lambda1-nocfg"
```
#######cfg-scale=1.8 modified for eval#########

#######have done lambda1\0.5 - layer5################
