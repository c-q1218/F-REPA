...
from loss import SILoss, FALoss, FA_D_Loss
...
def main(args):
    ...
    if args.resume_step > 0:
        ...
        ckpt = torch.load(
            f'/mnt/ssd/model/image/repa/FA/5-b-enc8-lambda1/checkpoints/{ckpt_name}',
            map_location='cpu',
            weights_only=False,
        )
                    ...
                    with accelerator.autocast():
                        for encoder, encoder_type, arch in zip(encoders, encoder_types, architectures):
                            ...
                            if 'dinov2' in encoder_type:
                                ...
                                z_2 = z[5]# mid layer x_norm_patchtokens(have done 5，3，7)
                            zs_1.append(z_1)
                            zs_2.append(z_2)

                with accelerator.accumulate(model):
                    model_kwargs = dict(y=labels)
                    loss, proj_loss = loss_fn(model, x, model_kwargs, zs_1=zs_1, zs_2=zs_2, dropout=drop)
                    loss_mean = loss.mean()
