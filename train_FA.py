...
from loss import SILoss, FALoss
...
def main(args):
    ...
    if args.resume_step > 0:
        print(f"loading resume model... ")
        ckpt_name = str(args.resume_step).zfill(7) + '.pt'
        ckpt = torch.load(
            f'/mnt/ssd/model/image/repa/FA/5-b-enc8-lambda0.75/checkpoints/{ckpt_name}',
            map_location='cpu',
            weights_only=False,
        )
            ...
            with torch.no_grad():
                ...
                with accelerator.autocast():
                    for encoder, encoder_type, arch in zip(encoders, encoder_types, architectures):
                        ...
                        z = encoder.forward_ret_all(raw_image_)
                        if 'dinov2' in encoder_type:
                            z_1 = z[11]# last layer x_norm_patchtokens
                            z_2 = z[9]# mid layer x_norm_patchtokens(have done 5，7)
                        zs_1.append(z_1)
                        zs_2.append(z_2)

            with accelerator.accumulate(model):
                model_kwargs = dict(y=labels)
                loss, proj_loss = loss_fn(model, x, model_kwargs, zs_1=zs_1, zs_2=zs_2)
                loss_mean = loss.mean()
                ...
                # # 设定阈值
                # threshold = 0.75
                # # 生成一个0到1之间的随机浮点数用于作为概率采样
                # x = random.random()
                # if x <= threshold:
                #     loss = loss_mean + proj_loss_mean * args.proj_coeff
                # else:
                #     loss = loss_mean + proj_loss_mean * args.proj_coeff * 0
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--batch-size", type=int, default=256)  # was 256, modified because of computation error
