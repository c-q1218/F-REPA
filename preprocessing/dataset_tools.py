...
def open_image_folder(source_dir, *, max_images: Optional[int]) -> tuple[int, Iterator[ImageEntry]]:
    ...
    for filename in os.listdir(source_dir):
        # 检查文件是否是图片文件（可以根据文件扩展名过滤）

        label_id = filename.split('_')[-1].split('.')[0]
        # 查找字典中对应的标签
        label = label_dict.get(label_id)
...
def convert(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        # label_id = archive_fname.split('_')[-1].split('.')[0]
        # label_description = label_dict.get(label_id, -1)
        # labels.append([archive_fname, label_description])
